...
(function(){
  ...
  async function pvLoad(){
    ...
    if (!_pvChannel) {
      _pvChannel = _supa.channel('pendencias_viewer_complexas_sync')
        .on('postgres_changes',
  ...
  function pvRenderModalTable(){
    ...
    if (!body) return;
    const rows = (_pvState.complexas || []).filter(r => r && r.modelo);
    ...
    if (!rows.length){
      body.innerHTML = `<div style="text-align:center;padding:60px 20px;color:var(--muted)">
        ...
        <div style="font-size:14px;font-weight:600">Nenhuma pendência complexa cadastrada no momento.</div>
      </div>`;
      return;
    }
